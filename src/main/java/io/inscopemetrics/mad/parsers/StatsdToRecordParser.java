...
import io.inscopemetrics.mad.model.statsd.StatsdType;
...
public final class StatsdToRecordParser implements Parser<List<Record>, ByteBuffer> {
    ...
    private ImmutableMap<String, String> parseInfluxStyleTags(@Nullable final String tagsAsString) {
        if (null != tagsAsString) {
            return ImmutableMap.copyOf(INFLUX_STYLE_TAG_SPLITTER.split(tagsAsString));
        }
        return ImmutableMap.of();
    }
    ...
    private static final Splitter.MapSplitter INFLUX_STYLE_TAG_SPLITTER = Splitter.on(',').withKeyValueSeparator('=');
