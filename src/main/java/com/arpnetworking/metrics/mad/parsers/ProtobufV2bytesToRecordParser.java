 ...
 */
public final class ProtobufV2bytesToRecordParser implements Parser<List<Record>, byte[]> {
    @Override
    public List<Record> parse(final byte[] data) throws ParsingException {
        try {
            final ClientV2.RecordSet request = ClientV2.RecordSet.parseFrom(ByteBuffer.wrap(data));
            ...
            throw new ParsingException("Could not create Request message from data", data, e);
        } catch (final ConstraintsViolatedException | IllegalArgumentException e) {
            throw new ParsingException("Could not build record", data, e);
        }
    }
