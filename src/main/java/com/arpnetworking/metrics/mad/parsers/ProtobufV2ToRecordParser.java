...
public final class ProtobufV2ToRecordParser implements Parser<List<Record>, HttpRequest> {
    ...
    public List<Record> parse(final HttpRequest data) throws ParsingException {
        try {
            final ClientV2.RecordSet request = ClientV2.RecordSet.parseFrom(data.getBody());
    ...
    private void processEntries(
        ...
        for (final ClientV2.MetricEntry metricEntry : entries) {
            ...
            for (final ClientV2.Quantity quantity : metricEntry.getSamplesList()) {
                final Number value;
                if (ClientV2.Quantity.ValueCase.DOUBLEVALUE.equals(quantity.getValueCase())) {
                    value = quantity.getDoubleValue();
                } else if (ClientV2.Quantity.ValueCase.LONGVALUE.equals(quantity.getValueCase())) {
                    value = quantity.getLongValue();
                } else {
                    throw new IllegalArgumentException("Quantity missing both doubleValue or longValue.");
                }

                // TODO(ville): Push Number representation down into Quantity.
                quantities.add(
    ...
    private Unit baseUnit(final ClientV2.CompoundUnit compoundUnit) {
        if (!compoundUnit.getNumeratorList().isEmpty()) {
            ...
            if (ClientV2.Unit.Type.Value.UNKNOWN.equals(selectedUnit.getType())
                    || ClientV2.Unit.Type.Value.UNRECOGNIZED.equals(selectedUnit.getType())) {
                return null;
            }
